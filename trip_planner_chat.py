...
app.config["SQLALCHEMY_DATABASE_URI"] = 'sqlite:///user_message_25.db'
...
class Hotel(db.Model):
    ...
    def __init__(self, message, vote):
        self.message= message
        self.vote = vote
...
@app.route('/up-vote', methods=['GET', 'POST'])
def upVote():

    print('hey hey')
    print('whaddup')
    #upvote the current number of Votes

    if (request.form['category'] == 'flight'):
        item = Flight.query.filter_by(message = request.form['message']).first()
        print(item)

    elif ((request.form['category'] == 'activity')):
        item = Activity.query.filter_by(message = request.form['message']['message']).first()
    elif ((request.form['category'] == 'food')):
        item = Food.query.filter_by(message = request.form['message']['message']).first()
    else:
        item = Hotel.query.filter_by(message = request.form['message']['message']).first()
...
def categoryUpdate():
    ...
    if (request.form['type'] == 'hotel'):
        print('in hotel')
        #initializing votes to 0
        entry = Hotel(message=request.form['message'], vote = 0)
    elif (request.form['type'] == 'flight'):
        print('in flight')
        entry = Flight(message=request.form['message'], vote = 0)
    elif (request.form['type'] == 'food'):
        entry = Food(message=request.form['message'], vote = 0)
    else:
        print('in activity')
        entry = Activity(message=request.form['message'], vote =0)
...
@app.route('/itinerary-confirmation', methods=['GET', 'POST'])
def getItinerary():

    #Flight
    top_flight = db.session.query(Flight).order_by(desc(Flight.vote)).first()

    #Hotel
    top_hotel = db.session.query(Hotel).order_by(desc(Hotel.vote)).first()

    #Activity
    top_activity = db.session.query(Activity).order_by(desc(Activity.vote)).first()

    #Food
    top_food = db.session.query(Food).order_by(desc(Food.vote)).first()

    #get top 3 of each section and send back to
    print('hello')
    print(top_food)
    print(top_flight)
    return(render_template('itinerary_confirmation.html', top_flight = top_flight, top_hotel = top_hotel, top_activity = top_activity, top_food = top_food))
...
def handle_my_category_update(json, methods=['GET', 'POST']):
    ...
    if (not json['need_recent_msg']):
        #check if already there or not in category database_file
        if (json['category'] == 'flight'):
            entry = Flight.query.filter_by(message=json['message']).first()
        elif (json['category'] == 'activity'):
            entry = Activity.query.filter_by(message=json['message']).first()

        elif (json['category'] == 'food'):
            entry = Food.query.filter_by(message=json['message']).first()
        #hotel
        else:
            entry = Hotel.query.filter_by(message=json['message']).first()
...
if __name__ == '__main__':
    create_connection("C:\\sqlite\db\user_message_25.db")
